...
class LSTM_attn(nn.Module):
    def __init__(self, vocab_size=5000, emb_dim=128, hid_dim=128, maps=None) -> None:
        ...
        self.w = nn.Parameter(torch.zeros(hid_dim * 2, self.charge_class_num))
        self.fc_input_dim = hid_dim*2*self.charge_class_num
    ...
    def forward(self, data):
        ...
        out=hidden*(alpha[:,:,0].unsqueeze(-1))
        for i in range(1,self.charge_class_num):
            tmp=hidden*(alpha[:,:,i].unsqueeze(-1))
